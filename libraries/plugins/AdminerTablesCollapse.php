...
class AdminerTablesCollapse
{
    ...
    public function __construct($separator = '__', $group_translations = true, $translation_sufixes = ['__translation','_translation','__translations','_translations'])
    {
    ...
    {
        ?>
        <script src="<?php echo asset('packages/adminer/js/jquery.min.js'); ?>" <?php echo nonce(); ?>></script>
        <script src="<?php echo asset('packages/adminer/js/jquery.cookie.min.js'); ?>" <?php echo nonce(); ?>></script>
    ...
    public function tablesPrint(array $tables)
    {
        ...
        $groups = $this->group_tables($tables);
    ...
    public function tables_print_line($line, $group_name, $actions, $cookie, $path)
    {
        $is_group = false;
        ...
        if ($is_group) {
            $group_name = ltrim($group_name, '_');
            $path .= $group_name;
            $active = (strpos($this->get_actual_table_name(), $path) === 0);
            foreach ($cookie as $c) {
            ...
            if (support('table') || support('indexes')) {
                ...
                echo '<a href="'.h(ME).'table='.urlencode($name).'"'.bold($active, (is_views($line) ? 'view' : ''))."' title=\"Select\" data-link='main'>$name</a>";
                echo '</div>';
            }
        }
    }

    public function group_tables(array $tables)
    {
        $groups = [];
    ...
    public function tables_group_translations(array $tables)
    {
        $final = [];
        $already_exists = [];
        foreach ($tables as $index => $actual_table) {
            if (! in_array($actual_table['Name'], $already_exists)) {
                ...
                foreach ($tables as $indextranslation => $ttranslation) {
                    foreach ($this->translation_sufixes as $sufix) {
                        if ($ttranslation['Name'] == $actual.$sufix) {
                            $already_exists[] = $ttranslation['Name'];
    ...
    public function get_actual_table_name()
    {
        $table = '';
